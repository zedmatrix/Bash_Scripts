...
_green="\e[1;32m"
_yellow="\e[1;33m"
print_green() { printf "${_yellow}"; printf "*** %s ***" "$*"; printf "${_reset}\n"; }
...
Encode () {
  ...
  #time ffmpeg "${log[@]}" -i "${input}" -vf "${deint[@]},${crop[@]},${scale[@]}" ${vcodec[@]} ${vbv[@]} \
  #-af ${loud[@]} ${acodec[@]} "${meta[@]}" "${output}"
}
# Convert input to output and title and gets duration just for infomation
String () {
